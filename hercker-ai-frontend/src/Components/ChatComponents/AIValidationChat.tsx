...
const AIValidationChat = observer(() => {
    const { globalLoading, validationResult, setValidationResult } = useContext(Context);
    ...
    const copyHandler = () => {
        if (validationResult) {
            copy(validationResult);
        }
    };

    return (
        <div className="chat-container ai-validation-chat">
            {globalLoading ? (
                <p>Loading ...</p>
            ) : (
                <>
                    <Editor
                        ...
                        onValueChange={(newCode) => setValidationResult(newCode)}
...
export default AIValidationChat;
